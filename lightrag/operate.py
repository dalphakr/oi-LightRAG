...
async def _apply_llm_context_filter(
    ...
    async def _run_relation_filter() -> tuple[list[dict], dict[str, Any]]:
        ...
        logger.debug(
            "[llm_filter] Relation filter start: input=%d batches=%d batch_size=%d history=%d",
            meta["input_count"],
            meta["batch_count"],
            relation_batch_size,
            len(query_param.conversation_history or []),
        )
        ...
        async def _run_relation_batch(
            ...
            logger.debug(
                "[llm_filter] Relation batch %d/%d prompt built in %dms (chars:%d items:%d)",
                batch_id,
                len(batches),
                prompt_ms,
                len(user_prompt),
            ...
            try:
                ...
                logger.debug(
                    "[llm_filter] Relation batch %d/%d error in %dms (prompt:%dms llm:%dms)",
                    batch_id,
                    len(batches),
                    total_ms,
                    prompt_ms,
            ...
            logger.debug(
                "[llm_filter] Relation batch %d/%d: %d -> %d (prompt:%dms llm:%dms parse:%dms total:%dms overhead:%dms)",
                batch_id,
                len(batches),
                len(batch_relations),
    ...
    async def _run_chunk_filter() -> tuple[list[dict], dict[str, Any]]:
        ...
        meta["batch_count"] = len(batches)
        logger.debug(
            "[llm_filter] Chunk filter start: input=%d batches=%d batch_size=%d history=%d",
            meta["input_count"],
            meta["batch_count"],
            chunk_batch_size,
            len(query_param.conversation_history or []),
        ...
        async def _run_chunk_batch(
            ...
            logger.debug(
                "[llm_filter] Chunk batch %d/%d prompt built in %dms (chars:%d items:%d)",
                batch_id,
                len(batches),
                prompt_ms,
                len(user_prompt),
            ...
            try:
                ...
                logger.debug(
                    "[llm_filter] Chunk batch %d/%d error in %dms (prompt:%dms llm:%dms)",
                    batch_id,
                    len(batches),
                    total_ms,
                    prompt_ms,
            ...
            overhead_ms = max(0, total_ms - llm_ms)
            logger.debug(
                "[llm_filter] Chunk batch %d/%d: %d -> %d (prompt:%dms llm:%dms parse:%dms total:%dms overhead:%dms)",
                batch_id,
                len(batches),
                len(batch_chunks),
                len(chunk_indices),
